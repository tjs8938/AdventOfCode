...
class State:
    ...
    def __init__(self, matrix: List[List[Dict[str, int]]], data_pos: Tuple[int, int]):
        self.matrix = matrix
            ...
            for x in range(len(self.matrix[y])):
                for direction in range(4):
                    ...
                    if 0 <= new_pos[0] < len(self.matrix[y]) and 0 <= new_pos[1] < len(matrix):
                        old_used = self.matrix[y][x]['used']
                        new_avail = self.matrix[new_pos[1]][new_pos[0]]['avail']
                        new_used = self.matrix[new_pos[1]][new_pos[0]]['used']
                        if old_used <= new_avail and new_used == 0:
                            s = State(deepcopy(self.matrix), self.data_pos)
                            s.steps = self.steps + 1
                            s.matrix[y][x]['used'] = 0
                            s.matrix[y][x]['avail'] = s.matrix[y][x]['size']
                            s.matrix[new_pos[1]][new_pos[0]]['avail'] = new_avail - old_used
                            s.matrix[new_pos[1]][new_pos[0]]['used'] += old_used
...
for input_line in open(filename).read().splitlines():
    ...
    if m is not None:
        ...
        del node['x']
        del node['y']
        nodes[position_to_key(x, y)] = node
