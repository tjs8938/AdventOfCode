...
def paint_panel(color):
    panels[pair_to_key(position)] = color
    if debug:
        print("Painting " + ("black" if color == 0 else "white"))


def scan_panel():
    color = panels.get(pair_to_key(position), 0)
...
printable = MatrixPrint.dict_to_matrix(panels, regex, xform=lambda x: '.' if x == 0 else '#')
MatrixPrint.print_matrix(printable)
