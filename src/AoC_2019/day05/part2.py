...
def input(modes):
    tape[get_param(False, modes[0])] = 5
...
def jump_if_true(modes):
    global pc
    value = get_param(True, modes[0])
    position = get_param(True, modes[1])
    if value != 0:
        pc = position
...
def less_than(modes):
    value1 = get_param(True, modes[0])
    value2 = get_param(True, modes[1])
    tape[get_param(False, modes[2])] = 1 if value1 < value2 else 0
