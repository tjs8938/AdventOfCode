...
def look_dir(direction, pos):
    next = pair_to_key(MOVE[direction](pos))
    return ship_map.get(next, None)


def move(direction):
    global move_count
    if look_dir(direction, droid_pos) is not None:
        return
...
time = 0
spaces_oxygenated = {oxygen_pos}

while len(spaces_oxygenated) > 0:

    newest_oxygenated = set()
    for space in spaces_oxygenated:
        for dir in range(0, 4):
            if look_dir(dir, space) == EMPTY:
                next_space = MOVE[dir](space)
                newest_oxygenated.add(next_space)
                ship_map[pair_to_key(next_space)] = OXYGEN

    if len(newest_oxygenated) > 0:
        time += 1

    spaces_oxygenated = newest_oxygenated
