...
class NonBlockingThreadedIntCodeComputer(threading.Thread):
    ...
    def input(self, modes):
        ...
        val = -1
        if self.has_packets():
            val = self.in_params[self.current_in_param_index]
            self.current_in_param_index += 1

        self.tape[self.get_param(False, modes[0])] = val
        self.mutex.release()

    def has_packets(self):
        return self.current_in_param_index < len(self.in_params)
    ...
    def parse_instruction(self):
        ...
        param_count = NonBlockingThreadedIntCodeComputer.param_counts[opcode]
